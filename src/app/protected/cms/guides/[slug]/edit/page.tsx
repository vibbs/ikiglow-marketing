...
export default function EditGuide({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadGuide() {
      try {
        const response = await fetch(`/api/cms/guides/${slug}`);
        if (!response.ok) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await removeContent({ type: "guides", slug });

      if (result.success) {
        alert("Guide deleted successfully!");
        router.push("/protected/cms/guides");
      } else {
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl tracking-wide">Edit Guide</h1>
              </div>
